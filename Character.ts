...
export class Character {
  ...
  update(keys: Keys, canvas: HTMLCanvasElement, deltaTime: number): void {
    ...
    if (keys["ArrowLeft"]) {
      this.dx = Math.max(
        ...
        Math.min(
          ...
          (this.dx *= this.momentumFactor * deltaTime)
        )
    ...
    // Initiate jump
    if ((keys["Space"] && this.jumpReleased) || this.bufferJump > 0) {
      if (keys["Space"]) {
      ...
      } else if (this.bufferJump <= 0) {
        this.bufferJump = this.settings.bufferFrames;
      }
    }
